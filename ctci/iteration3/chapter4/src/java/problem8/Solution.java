...
public class Solution {
    ...
    // is one node the common ancestor of the other
    // answer is limited to three possibilities 
    // - null
    // - node1
    // - node2
    public static <T> Node<T> firstCommonAncestor(Node<T> node1, Node<T> node2) {

        Node result = null;

        result = inorderTraversalSearch(node1, node2);

        if (result == null) {
            return inorderTraversalSearch(node2, node1);
        }
        
        return result;
    }

    public static <T> Node<T> inorderTraversalSearch(Node<T> root, Node<T> node2) {
        if (root == node2 || root == null) {
            return root;
        }

        Node node = inorderTraversalSearch(root.left, node2);
        if (node == null) {
            return inorderTraversalSearch(root.right, node2);
        }
    ...
    public static <T> Node<T> firstCommonAncestor(Node<T> root, Node<T> node1, Node<T> node2) {
        Node result = null;
        ...
        result = inorderTraversalSearch(node2, node1);
        ...
        // either the node1 and node2 share a parent that is not equal to node1 and not equal to node2
        // or there is no common ancestor (nodes are in different trees altogether).
        ArrayList<Node<T>> foundParent = new ArrayList<Node<T>>(1);
        searchForNodes(root, node1, node2, foundParent);

        if (foundParent.isEmpty()) {
            return null;
        }
        else {
            return foundParent.get(0);
        } 
    }
    ...
    private static class Result<T> {
        ...
        public Result(int leftValue, int rightValue) {
            this.leftValue = leftValue;
            this.rightValue = rightValue;
        }
    }

    private static <T> int searchForNodes(Node<T>root, Node<T>node1, Node<T>node2, ArrayList<Node<T>> result) {
        if (root == null) {
            return 0;
        }
        ...
        // want the first left is one and first right is one
        leftResult = searchForNodes(root.left, node1, node2, result); 
        rightResult = searchForNodes(root.right, node1, node2, result);

        if (leftResult == 1 && rightResult == 1) {
            // first parent where both are present
            result.add(root);
        } 
